...
router.post('/add', authenticate, async (req, res, next) => {
  try {
    ...
    if (isRazorpayMockEnabled()) {
      return res.json({
        razorpay: {
          keyId: 'mock_key',
          orderId: `order_mock_wallet_${Date.now()}`,
          amount: amountPaise,
          currency: 'INR',
...
router.post('/verify', authenticate, async (req, res, next) => {
  try {
    ...
    if (razorpay_signature === 'mock_signature' && !isRazorpayMockEnabled()) {
      return res.status(400).json({ error: 'Mock payments are not allowed in production' });
    }

    let paidAmount;

    if (isRazorpayMockEnabled() && razorpay_signature === 'mock_signature') {
      paidAmount = Number(bodyAmount);
      if (!paidAmount || paidAmount < 1 || paidAmount > 50000) {
        return res.status(400).json({ error: 'Invalid mock amount' });
      }
    } else {
      const rz = getRazorpay();
      ...
      paidAmount = Math.round((Number(payment.amount) / 100) * 100) / 100;
