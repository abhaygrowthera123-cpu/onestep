...
export const Wishlist = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api.getProducts({ isTrending: true, limit: 6 });
    ...
    <div className="max-w-7xl mx-auto px-4 py-12 sm:py-20">
      ...
      <div className="mt-32 space-y-12">
        <div className="flex items-end justify-between">
          ...
          <Link
            to="/search"
            className="group flex items-center space-x-2 text-slate-400 hover:text-blue-600 transition-colors font-black text-sm uppercase tracking-widest"
          >
            <span>View All</span>
        ...
        {trending.length > 0 ? (
          ...
          <p className="text-center text-slate-400 text-sm font-medium py-8">No trending products right now.</p>
        )}
      </div>
    </div>
  );
